...
public class RdsQueryHandler {
    ...
    private Response handleCreateDbInstance(MultivaluedMap<String, String> params, String region) {
        ...
        try {
            ...
            instance = service.setCreateTimeInstanceAttributes(id, instance.isStorageEncrypted(),
                    deletionProtection, instance.isAutoMinorVersionUpgrade(),
                    instance.isCopyTagsToSnapshot(), instance.getBackupRetentionPeriod(),
                    performanceInsightsEnabled);
            instance = service.setCreateTimeInstanceOptionalFields(id, null,
    ...
    private Response handleModifyDbInstance(
        ...
        try {
            ...
            instance = service.modifyInstanceOptionalFields(id, null,
                    monitoringInterval, monitoringRoleArn, performanceInsightsRetentionPeriod,
    ...
    private Response handleModifyDbParameterGroup(
        ...
        // Read both wire spellings: RDS's ParametersList overrides its member locationName
        // to "Parameter", but generic clients still send the default "member" form.
        for (String prefix : List.of("Parameters.Parameter.", "Parameters.member.")) {
            for (int n = 1; ; n++) {
                ...
                String applyMethod = params.getFirst(prefix + n + ".ApplyMethod");
                if (applyMethod != null) {
        ...
        try {
        DbParameterGroup group = service.modifyDbParameterGroup(name, parameters, applyMethods, region);
    ...
    private Response handleModifyDbClusterParameterGroup(
        ...
        Map<String, String> parameters = new HashMap<>();
        // Both wire spellings, matching handleModifyDbParameterGroup above.
        for (String prefix : List.of("Parameters.Parameter.", "Parameters.member.")) {
            for (int n = 1; ; n++) {
                String paramName = params.getFirst(prefix + n + ".ParameterName");
                if (paramName == null) {
