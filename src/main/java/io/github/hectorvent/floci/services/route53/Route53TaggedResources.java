 ...
 *
 * <p>A tagging-API write goes into Route 53's own tag store, the one {@code ChangeTagsForResource}
 * writes and {@code ListTagsForResource} reads, because that is where AWS keeps it.
 * {@code TagResources} on {@code arn:aws:route53:::hostedzone/Z123} is a change to the zone's tags.
 * The read side needs nothing here: {@code TaggedResourceScanner} already reads that store
 * ({@code route53-tags.json}) into the index.
 *
 * <p>The ARNs carry no region and no account, as on AWS.
 */
@ApplicationScoped
public class Route53TaggedResources implements TaggedResourceWriter {
