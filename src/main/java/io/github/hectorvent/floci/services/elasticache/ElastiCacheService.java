...
public class ElastiCacheService implements ResourceProvider {
    ...
    public CacheSubnetGroup createCacheSubnetGroup(String name, String description,
        ...
        synchronized (lockFor("sng:" + name)) {
            ...
            group.setArn(regionResolver.buildArn("elasticache",
                    regionResolver.getDefaultRegion(), "subnetgroup:" + name));
    ...
    /**
     * Resolves an ElastiCache tagging {@code ResourceName} to the resource it names. The
     * resource types match the ones {@code ListTagsForResource} reads: replication groups,
    ...
    private TagHandle resolveTagHandle(String resourceName) {
        ...
        String[] arn = resourceName.split(":", -1);
        if (arn.length != 7 || !"arn".equals(arn[0]) || !"elasticache".equals(arn[2])
                || arn[6].isBlank()) {
        ...
        return switch (type) {
            case "replicationgroup" -> {
                ReplicationGroup group = getReplicationGroup(name);
                yield new TagHandle(group.getTags(), updated -> {
                    ReplicationGroup current = getReplicationGroup(name);
                    current.setTags(updated);
                    groups.put(name, current);
                });
            }
            case "subnetgroup" -> {
                CacheSubnetGroup group = describeCacheSubnetGroups(name).getFirst();
                yield new TagHandle(group.getTags(), updated -> {
                    CacheSubnetGroup current = describeCacheSubnetGroups(name).getFirst();
                    current.setTags(updated);
                    subnetGroups.put(name, current);
                });
            }
            case "parametergroup" -> {
                CacheParameterGroup group = findParameterGroup(name)
                        .orElseThrow(() -> new AwsException("CacheParameterGroupNotFound",
                                name + " is not present", 404));
                yield new TagHandle(group.getTags(), updated -> {
                    CacheParameterGroup current = findParameterGroup(name)
                            .orElseThrow(() -> new AwsException("CacheParameterGroupNotFound",
                                    name + " is not present", 404));
                    current.setTags(updated);
                    parameterGroups.put(name, current);
                });
            }
            default -> throw new AwsException("InvalidARN",
