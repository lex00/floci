...
class RdsQueryHandlerTest {
    ...
    void createDbInstance_defaultsAutoMinorVersionUpgradeAndBackupRetentionWhenOmitted() {
        ...
        DbInstance instance = makeInstance("mydb");
        instance.setAutoMinorVersionUpgrade(true);
        instance.setBackupRetentionPeriod(1);
        when(service.createDbInstance(any(), any(), any(), any(), any(), any(), any(),
                anyInt(), anyBoolean(), any(), any(), any(), any(), anyBoolean(), anyBoolean(),
                any(), any(), anyList(), any(), any(), anyBoolean(), any(DbInstanceSettings.class)))
                .thenReturn(instance);
    ...
    void dbInstanceXml_echoesTheAuroraEngineIdentifierNotTheInternalFamily() {
        ...
        when(service.createDbInstance(eq("member1"), eq("aurora-mysql"), any(), any(), any(), any(), any(),
                ...
                any(), anyList(), any(), any(), anyBoolean(), any(DbInstanceSettings.class)))
                .thenReturn(instance);
        stubCreateTimeChain(instance);
    ...
    void createDbParameterGroup_passesTagsAndEchoesArn() {
        ...
        when(service.createDbParameterGroup(eq("pg1"), eq("postgres16"), eq("test group"), isNull()))
                .thenReturn(group);
        ...
        verify(service).createDbParameterGroup("pg1", "postgres16", "test group", null);
        verify(service).addTagsToResource("arn:aws:rds:us-east-1:000000000000:pg:pg1",
    ...
    void createDbSubnetGroup_passesTagsToService() {
        ...
        when(service.createDbSubnetGroup("sample-db-subnets", "test", List.of("subnet-aaa"),
                null, Map.of("Name", "sample-db-subnets")))
                .thenReturn(new DbSubnetGroup(
    ...
    void describeDbParameters_usesParameterTagNotMember() {
        ...
        when(service.getDbParameterGroup(eq("pg1"), any())).thenReturn(group);
    ...
    void describeDbClusterParameters_usesParameterTagNotMember() {
        ...
        when(service.getDbClusterParameterGroup("cpg1", null)).thenReturn(group);
    ...
    void modifyDbInstance_passesBackupSettingsToService() {
        ...
        org.mockito.Mockito.lenient().when(service.modifyInstanceOptionalFields(any(), any(), any(), any(),
                any(), any(), any(), any(), any())).thenReturn(instance);
        org.mockito.Mockito.lenient().when(service.applyRequestedPort(any(), any())).thenReturn(instance);
    ...
    private void stubCreateTimeChain(DbInstance instance) {
        org.mockito.Mockito.lenient().when(service.setCreateTimeInstanceAttributes(any(), anyBoolean(),
                anyBoolean(), anyBoolean(), anyBoolean(), anyInt(), anyBoolean())).thenReturn(instance);
        org.mockito.Mockito.lenient().when(service.setCreateTimeInstanceOptionalFields(any(), any(), any(),
                any(), any(), any(), any(), any())).thenReturn(instance);
        org.mockito.Mockito.lenient().when(service.applyCreateTimePort(any(), any())).thenReturn(instance);
    }
