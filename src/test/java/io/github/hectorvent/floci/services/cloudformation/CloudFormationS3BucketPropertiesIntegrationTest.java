 ...
 *
 * <p>Before this, the stack reported {@code CREATE_COMPLETE} for both and applied neither: the
 * bucket provisioner translated {@code CorsConfiguration} only, and the policy provisioner assigned
 * a physical id and stopped. A stack that says it succeeded for a bucket with versioning off is
 * the quiet kind of wrong - the template below is what a record-store bootstrap project deploys,
 * and its own verify step was what noticed.
 */
@QuarkusTest
