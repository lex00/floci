...
class CloudFormationDeletionPolicyIntegrationTest {
    ...
    void nestedStackIsPhysicallyDeletedWhenRemovedFromTemplateOnUpdate() throws InterruptedException {
        // Signed for s3 in the same account: a cloudformation-scoped signature on an S3 route
        // is (correctly) rejected by the known-scope fallthrough guard, and an unsigned PUT
        // would land the bucket in the default account instead of CUSTOM_AUTH's.
        String s3Auth = "AWS4-HMAC-SHA256 Credential=111122223333/20260205/eu-west-1/s3/aws4_request";
        given().header("Authorization", s3Auth).when().put("/nested-stack-templates").then().statusCode(200);
