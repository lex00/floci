...
class RdsServiceTest {
    ...
    void dbParameterGroupTagsRoundTripByArn() {
        DbParameterGroup group = rdsService.createDbParameterGroup("pg-tags", "postgres16", "test group",
                "us-east-1");
        rdsService.addTagsToResource(group.getDbParameterGroupArn(), java.util.Map.of("Name", "pg-tags"));
    ...
    void createDbInstanceSucceedsAsMetadataWhenNoDockerDaemonIsReachable() {
        when(containerManager.tryStart(any(), any(), any(), any(), any(), any(), any(), any(), any())).thenReturn(null);
    ...
    void backendIsRetriedOncePerCallSoTheContainerStartsWhenADaemonAppears() {
        ...
        verify(proxyManager).startProxyPreferring(any(), eq(DatabaseEngine.POSTGRES), eq(false),
                any(), eq(started.getProxyPort()), eq("127.0.0.1"), eq(15432), any(),
    ...
    void createDbSubnetGroupPersistsTagsFromCreateRequest() {
        ...
        DbSubnetGroup group = rdsService.createDbSubnetGroup("my-subnet-group", "desc",
                List.of("subnet-default-a", "subnet-default-b"),
                "us-east-1", Map.of("Name", "my-subnet-group"));
    ...
    void sameNamedInstancesInDifferentAccountsUseIndependentRelayKeys() {
        ...
        verify(proxyManager).startProxyPreferring(eq(relayKeyA), any(), anyBoolean(), any(), anyInt(),
                any(), anyInt(), any(), any(), any(), any(), any());
        verify(proxyManager).startProxyPreferring(eq(relayKeyB), any(), anyBoolean(), any(), anyInt(),
                any(), anyInt(), any(), any(), any(), any(), any());
