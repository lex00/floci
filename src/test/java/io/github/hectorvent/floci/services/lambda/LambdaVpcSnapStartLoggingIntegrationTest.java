...
class LambdaVpcSnapStartLoggingIntegrationTest {
    ...
    void updateFunctionConfigurationDetachesVpcConfigAndClearsVpcId() {
        ...
        .then()
            .statusCode(200)
            // Real AWS reports a detached function with an emptied VpcConfig struct
            // (empty SubnetIds/SecurityGroupIds, no VpcId), not an absent key.
            .body("VpcConfig.SubnetIds", empty())
            .body("VpcConfig.VpcId", nullValue());
