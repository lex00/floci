...
class Ec2VpcEndpointSecurityGroupConsumerTest {
    ...
    void modifyVpcEndpoint_resetPolicy_clearsPolicyDocument() {
        ...
        .then()
            .statusCode(200)
            // Resetting restores the default full-access document; AWS never describes an
            // endpoint without a policy, so the custom document is replaced, not removed.
            .body("DescribeVpcEndpointsResponse.vpcEndpointSet.item.policyDocument",
                    not(equalTo("{\"Version\":\"2012-10-17\"}")))
            .body("DescribeVpcEndpointsResponse.vpcEndpointSet.item.policyDocument",
